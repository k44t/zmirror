...
zfs_blockdev_id = "ZDevCache|zpool-a|partition-a"
...
class Test_Group1_TestMethods():
  ...
  def test_zdev_online(self):
    ...
    dev: ZDevCache = config.cache_dict[zfs_blockdev_id]
    assert dev.state.what == EntityState.DISCONNECTED
    ...
    dev2: ZDevCache = config.cache_dict[zfs_blockdev_id]
  ...
  def test_zdev_disconnected(self):
    ...
    dev: ZDev = config.cache_dict[zfs_blockdev_id]
